<template>
  <div>
              ...
              <ul>
                <li v-for="(address,index) in addressListFilter" :class="{'check': index == checkIndex}" @click="checkAddress(index, address.addressId)">
                  ...
                  <div class="addr-opration addr-del">
                    <a href="javascript:;" class="addr-del-btn"  @click="deleteBtn(address.﻿﻿addressId)">
    ...
    <model :mdShow="isMdShow" @close="closeModal">
      ...
      <div slot="btnGruop">
        <a class="btn btn--m" href="javascript:;" @click="deleteAddress" >确认</a>
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      deleteAddress() {
        axios.delete('/api/users/addressList/' + this.deleteAddressId ).then(res => {
          if ( !res.data.status) {
