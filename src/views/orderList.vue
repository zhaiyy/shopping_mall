              ...
              <li v-for="order in orderList">
                <div class="cart-tab-1">
                  <div class="cart-item-pic">
                    <img v-lazy="'/static/'+ order.prodcutImg" >
                  </div>
                ...
                <div class="cart-tab-2">
                  <div class="item-price">${{order.prodcutPrice|currency}}</div>
                </div>
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">${{order.prodcutPrice * order.productNum | currency}}</div>
                </div>
              </li>
            </ul>
          </div>
    ...
    computed: {
      subtotal() {
        ...
        this.orderList.forEach(ele => {
          total += ele.prodcutPrice * ele.productNum
        })
        return total
      },
      orderTotal() {
    ...
    methods: {
      getCartList() {
        axios.get('/api/users/cartList').then(res => {
          this.orderList = res.data.result.filter(ele => {
            return ele['checked']
          })
        })
      },
